...
input_frame.grid(row=0,column=0,rowspan=5,columnspan=5)
...
crm_tag      =Entry(input_frame,width=50,borderwidth=2,fg="black",font=('Consolas',14))
...
crm_qr_code=Entry(input_frame,width=50,borderwidth=2,fg="black",font=('Consolas',14))
crm_qr_code.grid(row=5, column=1,columnspan=2)
...
trv.column("#3",anchor="w", width=200, stretch=False)
...
def load_json_from_file():
    ...
    with open("..\\SAB (AI Assistant)\\json_files\\fil_intents.json","r") as file_handler:
...
def save_json_to_file():
    ...
    with open("..\\SAB (AI Assistant)\\json_files\\fil_intents.json", "w") as file_handler:
