...
crm_patterns      =Entry(input_frame,width=30,borderwidth=2,fg="black",font=('Consolas',14))
crm_patterns.grid(row=3, column=1,columnspan=2)

crm_responses=Entry(input_frame,width=30,borderwidth=2,fg="black",font=('Consolas',14))
...
def load_json_from_file():
    ...
    with open("json-files\\intents.json","r") as file_handler:
...
def save_json_to_file():
    ...
    with open("json-files\\intents.json", "w") as file_handler:
...
def load_trv_with_json():
    ...
    for key in jsonData:
        ...
        trv.insert('',index='end',iid=rowIndex,text="",
                        values=(guid_value,tag,patterns,responses))
        rowIndex=rowIndex+1
...
def add_entry():
    ...
    process_request('_INSERT_',guid_value,tag,patterns,responses) 
...
def update_entry():
    ...
    process_request('_UPDATE_',guid_value,tag,patterns,responses)  
...
# --review=Y
def process_request(command_type,guid_value,tag,patterns,responses):
    global jsonData 
    ...
    if command_type == "_UPDATE_":
        ...
        if row >= 0:
            ...
            dict = {"id":guid_value, "tag":tag, 
                    "patterns":patterns.split("//"), "responses":responses.split("//")} 
            jsonData[row]=dict 
